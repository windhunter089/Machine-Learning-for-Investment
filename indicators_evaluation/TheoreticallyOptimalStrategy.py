...
def test_benchmark(symbol, sd, ed, sv):
    benchmark_df = pd.DataFrame(columns=['Date', 'Symbol', 'Order', 'Shares'])
    benchmark_df = benchmark_df.set_index('Date')
    prices = get_data([symbol], pd.date_range(sd, ed), addSPY=False)
    prices = prices.dropna(how='any', subset=[symbol])
    benchmark_df.loc[prices.index[0]] = [symbol,'BUY',1000]
    benchmark_df.loc[prices.index[-1]] = [symbol,'SELL',1000]
